        ...
        <header className="flex flex-col gap-6 rounded-3xl border border-amber-300/10 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(255,215,0,0.06),0_20px_80px_rgba(0,0,0,0.55)] backdrop-blur">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex w-full flex-wrap items-center gap-2">
              {(["AED", "USD", "SAR"] as CurrencyKey[]).map((k) => {
                const on = k === active;
                return (
                  <button
                    key={k}
  ...
  سعر الصرف: 1 USD = {rates.AED ? rates.AED.toFixed(4) : "—"} AED • {rates.SAR ? rates.SAR.toFixed(4) : "—"} SAR
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            ...
            <div className="mt-5 grid gap-3">
              <InfoCard
                title="عيار 24"
                desc="أنقى ذهب (تقريباً 99.9%)."
              />
              <InfoCard
                title="عيار 22"
                desc="شائع في الخليج للمجوهرات."
              />
              <InfoCard
                title="عيار 21"
                desc="الأكثر تداولاً في كثير من الأسواق."
              />
              <InfoCard
                title="عيار 18"
                desc="مناسب للتصاميم الحديثة ومقاوم للخدش أكثر."
              />
            </div>
            ...
            <div className="mt-6 rounded-2xl border border-amber-200/15 bg-amber-300/10 p-4 text-sm text-amber-100">
              ...
              <p className="mt-1 text-amber-100/90">
                تحويل العملات هنا ثابت (تقريبي). إذا تبغى نخليه “لايف” من سعر الصرف الحقيقي، أضيفه لك.
              </p>
            </div>
          </div>
        </section>
